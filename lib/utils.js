...
module.exports = {
  ...
  async setConfig(data) {
    return new Promise((resolve, reject) => {
      fs.writeFile("./.bombino", JSON.stringify(data), err => {
        if (err) reject(err);
        resolve(true);
      });
    });
  ...
  generateFinalizedConfig(config, type, name) {
    Object.keys(config[type]).map((key, i) => {
      config[type][key] = config[type][key].replace(/^\./, `./${name}`);
    });
    return config[type];
  },
  ...
  async _correctPlaceholders(SETTINGS) {
    ...
    console.log("Rewrite this via config");
    return SETTINGS;
  },
  //
  // OLD
