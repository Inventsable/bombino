...
module.exports = {
  ...
  async getStaticTemplates() {
    return [
      {
        gitURL: "Inventsable/bombino-brutalism-basic",
        index: 0,
        label: "Vue-CLI, brutalism, basic utilities",
        model: "VUE",
        name: "bombino-brutalism-basic",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-brutalism-router",
        index: 1,
        label: "Vue-CLI, brutalism, router",
        model: "VUE",
        name: "bombino-brutalism-router",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-brutalism-vuex",
        index: 2,
        label: "Vue-CLI, brutalism, router, vuex",
        model: "VUE",
        name: "bombino-brutalism-vuex",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-vue-bare",
        index: 3,
        label: "Absolute minimum",
        model: "VUE",
        name: "bombino-vue-bare",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-vue-basic",
        index: 4,
        label: "Include utility components",
        model: "VUE",
        name: "bombino-vue-basic",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-vue-router",
        index: 5,
        label: "Basic and Vue Router",
        model: "VUE",
        name: "bombino-vue-router",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-vue-vuetify",
        index: 6,
        label: "Vuetify, Router, Vuex",
        model: "VUE",
        name: "bombino-vue-vuetify",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-quasar-plus",
        index: 7,
        label: "Quasar, Vuex, Modal, Lottie",
        model: "QUASAR",
        name: "bombino-quasar-plus",
        path: null,
      },
      {
        gitURL: "Inventsable/bombino-quasar-panelify",
        index: 8,
        label: "Quasar, Vuex, Panelify",
        model: "QUASAR",
        name: "bombino-quasar-panelify",
        path: null,
      },
    ];
  },
  ...
  _constructDebugText(settings) {
    return new Promise((resolve) => {
      let mirror = "";
      const hasModal = /plus|slim|vuetify/.test(settings.templateType);
      let prefix = `<Extension Id="com.${settings.id}.panel">
        <HostList>
        `;
      let str = prefix + "";
      let suffix = `</HostList>
      ...
      settings.hostList.forEach((item, i) => {
        ...
        str += `    <Host Name="${host.name}" Port="${portNum}" />
        `;
      });
      str += suffix;
    ...
    <Extension Id="com.${settings.id}.modal">
        ...
        settings.hostList.forEach((item, i) => {
          ...
          str += `    <Host Name="${host.name}" Port="${portNum + 1}" />
        `;
        });
        str += suffix;
      }
  ...
  async setConfigDirty(SETTINGS) {
    ...
    config = this.sanitizeFinalizedConfig(
      config,
      SETTINGS.model,
      SETTINGS.extName.split(" ").join("-").toLowerCase()
    );
            ...
            if (/name/i.test(pKey))
              ...
              FILE_DATA = FILE_DATA.replace(
                ...
                /package|manifest/.test(key)
                  ? keymatch.split(" ").join("-").toLowerCase()
                  : keymatch
              );
