...
module.exports = {
  ...
  _downloadGitRepo: function(settings) {
    return new Promise((resolve, reject) => {
      download(
        `${settings.location}`,
        settings.extName.split(" ").join("-"),
  ...
  getLocalTemplates: async function() {
    let contents = fs.readdirSync("./");
    return contents.includes(".bombino")
      ? JSON.parse(fs.readFileSync("./.bombino", { encoding: "utf-8" }))
      : false;
  },
  setLocalTemplates: async function(templates) {
    return new Promise((resolve, reject) => {
      fs.writeFile("./.bombino", JSON.stringify(templates), err => {
        if (err) reject(err);
        resolve(true);
      });
    });
    // return fs.writeFileSync("./.bombino", JSON.stringify(templates));
  },
  readDir: async function(thispath) {
    return new Promise((resolve, reject) => {
      fs.readdir(
  ...
  addLocalTemplate: async function(data) {
    let db = this.getLocalTemplates();
    if (db) {
      return this.setLocalTemplates(db.push(data));
    } else {
      return this.setLocalTemplates([data]);
    }
  },
  ...
  // Should probably allow a CONFIG option for customizable paths/values?
  // Not sure if that would be overkill
  //
