...
const bombino = {
  ...
  newAdobePanelPrompt: async function () {
    ...
    if (!hasLocal && isLocal) {
      console.log("No local templates were found!");
      this.kill();
    }
  ...
  async endCreateTemplate(SETTINGS, spinner) {
    ...
    if (optionalAdd.save) {
      console.log(
        ...
        )} is now available in ${chalk.blue("local")} > ${chalk.blue(
          `${SETTINGS.model}`
        )} during new ${chalk.yellow("bombino")} commands.`
      );
    }
  ...
  async endCreatePanel(SETTINGS, spinner) {
    ...
    this.readyMessage(SETTINGS);
    // console.log('');
    console.log(`Ready to get started? Run the following commands:`);
    console.log("");
